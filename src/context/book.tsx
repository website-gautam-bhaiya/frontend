...
export default function BooksProvider({ children }: { children: ReactNode }) {
  ...
  const getAllBooks = async () => {
    try {
      ...
      if (response.status === 200 && response.data.books) {
        setBooks(response.data.books); // Safe assignment of books
      } else {
  ...
  const publishBook = async (data: {
    ...
    try {
      console.log("Publishing Book Data:", data);
  ...
  return (
    <BooksContext.Provider value={{ books, setBooks, getAllBooks, publishBook }}>
      {children}
